...
## Vendored builders were removed

`rust/op-rbuilder/` and `rust/rollup-boost/` are gone, along with their images, apko/melange
definitions, CI jobs, devstack launchers, and the `SingleChainWithFlashblocks` preset. No public
live subblock producer replaces them.

Preserved deliberately: op-conductor's *external* rollup-boost client, health, and WebSocket
integrations, including the `op-service/client` WebSocket helper they build on;
`reth-optimism-flashblocks`; op-alloy flashblock wire types; and operator documentation pointing at
supported external rollup-boost.

`op-service/sources/flashblock_client.go` went with them: the deleted flashblocks acceptance tests
were its only callers, so keeping it would have left an unreferenced stream parser behind. A future
in-repo flashblocks consumer should re-derive one against the then-current wire types.

Deployments outside this repository that consumed the monorepo's `op-rbuilder` / `rollup-boost`
images must pin an existing image before consuming a revision that no longer publishes them.
...
rollback against a real policy; and the flashblocks live-stream tests that depended on the vendored
builders.
